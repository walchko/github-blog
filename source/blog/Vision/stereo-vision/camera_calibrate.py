...
class StereoCalibration(object):
    ...
    def cal(self, images):
        for i, gray in enumerate(images):
            ...
            if ret is True:
                rt = cv2.cornerSubPix(gray, corners, (11, 11),(-1, -1), self.criteria)
                self.imgpoints.append(corners)

                # Draw and display the corners
                # ret = cv2.drawChessboardCorners(img, (9, 6),corners, ret)
                # cv2.imshow(images_left[i], img_l)
                # cv2.waitKey(500)

            img_shape = gray.shape[::-1]

        rt, M1, d1, r1, t1 = cv2.calibrateCamera(objpoints, imgpoints, img_shape, None, None)

    def calibrate(self, images_left, images_right):
        for i, gray_l, gray_r in enumerate(zip(images_left, images_right)):
