...
- [Imports](https://github.com/walchko/github-blog/blob/master/source/blog/swift/stanford/cs193p.md#imports)
- [Try](https://github.com/walchko/github-blog/blob/master/source/blog/swift/stanford/cs193p.md#try)
...
| `@escaping`        | |
| `@EnvironmentalObject` | an efficient way to pass an `ObservedObject` tracked by a root, to a deeply nested child without having to pass the object through the entire tree to the child. Said another way, Think of `@EnvironmentObject` as a smarter, simpler way of using `@ObservedObject` on lots of views. Rather than creating some data in view A, then passing it to view B, then view C, then view D before finally using it, you can create it in view A and put it into the environment so that views B, C, and D will automatically have access to it.  |
| `@discardableResult` | |
...
| `defer`        | execute code at end of scope, useful for clean-up actions like closing files when done |
...
- AdvancedSwift.com: [Learn Swift for C++ Developers](https://www.advancedswift.com/learn-swift-cpp/)
- hackingwithswift.com: [@EnvironmentObject](https://www.hackingwithswift.com/quick-start/swiftui/how-to-use-environmentobject-to-share-data-between-views)
